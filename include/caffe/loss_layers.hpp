 ...
 */
template <typename Dtype>
class MapPredictionLayer : public Layer<Dtype> {
 public:

  explicit MapPredictionLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "MapPrediction"; }
  virtual inline int ExactNumBottomBlobs() const { return 3; } // 3 bottoms  [fc8, real_label(label), meta_label]
  ...
  virtual const  vector<int> get_labels(); // real labels
  virtual const  vector<int> get_meta_labels();
  virtual const  vector<int> get_predicted_labels();
  virtual const  vector< vector<Dtype> > get_predicted_probs();
  virtual const  vector< vector<Dtype> > get_predicted_losses();
 ...
 protected:
  ...
  vector<int> predicted_labels_;
  vector<int> meta_labels_;
  vector<int> labels_; // real labels

  // predicted values(probs or output of previous layer).
  // 1 vector (1 row) for each predicted label
  // size = num_of_current_samplesXnum_of_meta_labels
  vector< vector<Dtype> > predicted_probs_;

  vector< vector<Dtype> > predicted_losses_;
...
/**
 * @brief Computes the classification accuracy for a one-of-many
 *        classification task as AccuracyLayer, but use mapped file to
 *        map predicted class_id to another class_id.
 */
template <typename Dtype>
class MapAccuracyLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides MapAccuracyParameter map_accuracy_param,
   *     with AccuracyLayer options:
   *   - top_k (\b optional, default 1).
   *     Sets the maximum rank @f$ k @f$ at which a prediction is considered
   *     correct.  For example, if @f$ k = 5 @f$, a prediction is counted
 ...
 protected:
  ...
  // Which label to map, (which bottom value, 0 (predicted) or 1(groundTruth) ).
  int label_bottom_index_;
  string out_accuracy_results_file_;
  string out_predictions_results_file_;

  // Map vector
  string map_file_;
  vector<int> map_labels_;
};



/**
 * @brief Computes the Expert classification accuracy for a one-of-many
 *        classification task.
 */
template <typename Dtype>
class ExpertAccuracyLayer : public Layer<Dtype> {
 public:

  explicit ExpertAccuracyLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "ExpertAccuracy"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
...
/*
 *
 *Karim: Expert SoftmaxWithLoss Layer
 *
 *
*/
...
template <typename Dtype>
class ExpertSoftmaxWithLossLayer : public LossLayer<Dtype> {
 public:

  explicit ExpertSoftmaxWithLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "ExpertSoftmaxWithLoss"; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }

 protected:
  /// @copydoc ExpertSoftmaxWithLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
 /* virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
...
*/
  ...
  // Apply expert_loss_weight_ * loss, if current_label = expert_loss_weight_label_
  bool has_dustbin_class_label_;
  int dustbin_class_label_;
  float dustbin_loss_weight_;
  float others_loss_weight_;
