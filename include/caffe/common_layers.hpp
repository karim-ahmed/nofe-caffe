...
/**
 * @brief As ArgMax, but with map file. Compute the index of the @f$ K @f$ max values for each datum across
 *        all dimensions @f$ (C \times H \times W) @f$.
 *
 * Intended for use after a classification layer to produce a prediction.
 * If parameter out_max_val is set to true, output is a vector of pairs
...
class MapArgMaxLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  // Map vector
   ...
   // used to shift labels in map file
   // increment label_shift_value for all labels
   int label_shift_value_ ;
};
